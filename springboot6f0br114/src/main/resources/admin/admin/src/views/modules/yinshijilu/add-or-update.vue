	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="餐食来源" prop="canshilaiyuan">
					<el-select :disabled="ro.canshilaiyuan" v-model="ruleForm.canshilaiyuan" placeholder="请选择餐食来源" multiple filterable>
						<el-option
							v-for="(item,index) in canshilaiyuanOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="餐食来源" prop="canshilaiyuan">
					<el-input v-model="ruleForm.canshilaiyuan"
						placeholder="餐食来源" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="饮食内容" prop="yinshineirong">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.yinshineirong" label="饮食内容" prop="yinshineirong">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.yinshineirong}}</span>
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='canshilaiyuan'){
							this.ruleForm.canshilaiyuan = obj[o];
							this.ro.canshilaiyuan = true;
							continue;
						}
						if(o=='yinshineirong'){
            ...
            this.canshilaiyuanOptions = "自制,食堂,外卖,外食".split(',')
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.canshilaiyuan = this.ruleForm.canshilaiyuan.split(",");
        } else {
          this.$message.error(data.msg);
        }
      });
      ...
      //更新跨表属性
       ...
       if(this.type=='cross'){
		...
		this.$refs["ruleForm"].validate(valid => {
			if (valid) {
				this.ruleForm.canshilaiyuan = this.ruleForm.canshilaiyuan.join(",");
