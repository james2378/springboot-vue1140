<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('jiankangxinxi','健康数据')" type="success" @click="chartDialog1()">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yonghumingChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var yonghumingChart1 = echarts.init(document.getElementById("yonghumingChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yonghumingChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yonghumingChart1.resize();
                };
            }
        });
      })
