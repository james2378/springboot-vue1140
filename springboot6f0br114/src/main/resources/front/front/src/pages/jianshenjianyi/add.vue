...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="建议时间" prop="jianyishijian">
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="健身建议" prop="jianshenjianyi">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="健身建议"
              v-model="ruleForm.jianshenjianyi">
            </el-input>
          </el-form-item>
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          jianshenjianyi: [
            { required: true, message: '健身建议不能为空', trigger: 'blur' },
          ],
          jianyishijian: [
          ],
        },
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='jianshenjianyi'){
              this.ruleForm.jianshenjianyi = obj[o];
              this.ro.jianshenjianyi = true;
              continue;
            }
            if(o=='jianyishijian'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jianshenjianyi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jianshenjianyi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jianshenjianyi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
